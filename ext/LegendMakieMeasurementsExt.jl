...
module LegendMakieMeasurementsExt
    ...
    function LegendMakie.lplot!(
        ...
        Makie.band!(ax, [xlims...], (Measurements.value(report.miny) .+ (-1,1) .* Measurements.uncertainty(report.miny))..., color = (:red,0.2))
-        
        legend_position != :none && Makie.axislegend(ax, position = legend_position)
    ...
    function LegendMakie.lplot!(report::NamedTuple{(:wl, :min_obj, :gain, :res_1pe, :pos_1pe, :threshold, :a_grid_wl_sg, :obj, :report_simple, :report_fit)}; kwargs...)
        LegendMakie.lplot!(
            ...
            ylabel = latexstring("\\fontfamily{Roboto} Objective\\; \$\\frac{\\sigma \\cdot \\sqrt{\\text{threshold}}}{\\text{gain} \\cdot \\sqrt{\\text{pos_-1pe}}}\$"), 
