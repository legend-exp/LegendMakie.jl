...
# histogram is the same as `lhist`. Every plot carries the Juleana logo and a `PRELIMINARY`
# tag until it is marked `final`.
...
# ## Watermarks
#
# `add_watermarks!` adds the logo and tags to the current figure. It is called by every
# recipe unless `watermark = false`; `final = true` drops the `PRELIMINARY` tag. The
# LEGEND logo can replace the Juleana one.

fig = Figure()
ax = Axis(fig[1, 1], xlabel = "x", ylabel = "y")
lines!(ax, 0:0.1:10, sin)
LegendMakie.add_watermarks!(legend_logo = true, position = "outer top", preliminary = false)
fig

# `residualplot!` draws normalized residuals with the bands of one and three standard deviations.

fig = Figure(size = (600, 200))
ax = Axis(fig[1, 1], xlabel = "x", ylabel = "Residuals (σ)")
LegendMakie.residualplot!(ax, (x = 1:50, residuals_norm = randn(50)))
fig
